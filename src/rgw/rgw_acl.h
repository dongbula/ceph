...
public:
  ...
  int get_perm(rgw_user& id, int perm_mask, const char * http_referer);
  int get_group_perm(ACLGroupTypeEnum group, int perm_mask);
  bool verify_permission(rgw_user& uid, int user_perm_mask, int perm, const char * http_referer = nullptr);
